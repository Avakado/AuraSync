...
import { Sparkles, Brain, Activity, Zap, CheckCircle2, RefreshCw, AlertCircle, Send, Radio } from "lucide-react";
import { NeuroManifestationResult, VibrationQuizAnswers } from "../types";

export default function VibrationCalculator() {
  const [step, setStep] = useState(0); // 0: intro, 1: desire, 2: energy, 3: focus, 4: obstacle, 5: loading, 6: result
  ...
  const calculateSync = async (finalObstacle: string) => {
    ...
    try {
      ...
      console.error(err);
      setError("Unable to sync frequency via neural net. Restructured default active.");
      // Fallback
      setResult({
        currentFrequencyHz: 432,
        frequencyBandName: "Alpha-Coherent Harmonic",
        rasStatus: "Active calibration requested",
        neuroplasticInsight: "Your default mode network is processing high analytical filters. Pruning excess feedback loops is advised.",
        metaphysicalBridging: "A shift towards non-binary focus will release sub-harmonic anchors, establishing rapid wave alignment.",
        actionSteps: [
        ...
        customAffirmation: "I release the friction of thought to align directly with my intention."
      });
      setStep(6);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="w-full bg-surface border border-stroke rounded-3xl p-6 md:p-8 relative overflow-hidden group/calc" id="vibe-calculator">
      {/* Background Aura Glow effect */}
      <div className="absolute -right-24 -top-24 w-48 h-48 rounded-full bg-accent-light/10 blur-[60px] pointer-events-none group-hover/calc:bg-accent-dark/15 transition-all duration-700" />
        ...
        {step === 0 && (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] text-muted uppercase tracking-[0.2em] font-mono block">BIOMETRIC FIELD INDEX</span>
                <h4 className="text-lg font-medium text-text-primary">Current Vibration Calculator</h4>
              </div>
            </div>

            <p className="text-sm text-muted leading-relaxed">
              Measure your neuro-cognitive configuration. This lightweight multi-step engine maps your Reticular Activating System (RAS) filters to provide instant bio-metaphysical alignment insights.
            </p>
        ...
        {step === 1 && (
          ...
          >
            <div className="space-y-1">
              ...
              <h4 className="text-base font-medium text-text-primary">What is your current manifestation desire?</h4>
              <p className="text-xs text-muted">State your primary intention simply (abundance, career leap, inner peace).</p>
            </div>
            ...
            <button
              ...
              id="step-1-next"
            >
              Continue Calibration <Send className="w-3.5 h-3.5 text-accent-light shrink-0" />
            </button>
          </motion.div>
        )}
        ...
        {step === 2 && (
          ...
          >
            <div className="space-y-1">
              ...
              <h4 className="text-base font-medium text-text-primary font-sans">Index your overall somatic energy level right now:</h4>
            </div>
        ...
        {step === 3 && (
          ...
          >
            <div className="space-y-1">
              ...
              <h4 className="text-base font-medium text-text-primary">What is your current attention capability?</h4>
            </div>
        ...
        {step === 4 && (
          ...
          >
            <div className="space-y-1">
              ...
              <h4 className="text-base font-medium text-text-primary">Identify your primary neurological restriction:</h4>
            </div>
        ...
        {step === 5 && (
          ...
          >
            ...
            <div className="text-center">
              <p className="text-xs font-mono text-muted tracking-widest uppercase">Calculating Synaptic Alignment</p>
              <p className="text-[11px] text-muted/60 mt-1">Interpreting RAS filters via deep neuroplasticity models...</p>
            </div>
          </motion.div>
        )}
        ...
        {step === 6 && result && (
          ...
          >
            ...
            <div className="flex justify-between items-start border-b border-stroke pb-4">
              <div>
                <span className="text-[10px] text-accent-light uppercase tracking-widest font-mono">CALIBRATION SYNCHRONIZED</span>
                <h5 className="text-xl font-display italic text-text-primary text-gradient">{result.frequencyBandName}</h5>
              </div>
              <div className="text-right">
                <span className="text-xs text-muted block font-mono">FREQUENCY</span>
                <span className="text-2xl font-display font-bold text-text-primary tracking-tight">{result.currentFrequencyHz} Hz</span>
              </div>
            </div>
            ...
            <div className="space-y-3.5">
              <div className="bg-bg/40 border border-stroke p-3 rounded-xl space-y-1">
                <span className="text-[9px] font-mono text-muted uppercase tracking-wider block">RAS FILTRATION ACCURACY</span>
                <p className="text-xs text-text-primary font-medium">{result.rasStatus}</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="bg-bg/40 border border-stroke p-3 rounded-xl space-y-1">
                  <span className="text-[9px] font-mono text-muted uppercase tracking-wider block">NEUROPLASTIC MATRIX</span>
                  <p className="text-[11px] text-muted leading-relaxed">{result.neuroplasticInsight}</p>
                </div>
                <div className="bg-bg/40 border border-stroke p-3 rounded-xl space-y-1">
                  <span className="text-[9px] font-mono text-muted uppercase tracking-wider block">METAPHYSICAL COLLAPSE</span>
                  <p className="text-[11px] text-muted leading-relaxed">{result.metaphysicalBridging}</p>
                </div>
              </div>

              {/* Action routines */}
              <div className="space-y-2">
                <span className="text-[10px] font-mono text-muted uppercase tracking-wider block">RECOMMENDED SYNC ROUTINES</span>
                <div className="space-y-1.5">
                  {result.actionSteps.map((stepStr, idx) => (
                    <div key={idx} className="flex gap-2.5 items-start text-[11px] text-muted bg-stroke/10 px-3 py-2 rounded-lg">
              ...
              <div className="bg-accent-dark/5 border border-accent-light/10 p-4 rounded-xl relative overflow-hidden text-center">
                ...
                <span className="text-[9px] font-mono text-accent-light uppercase tracking-widest block mb-1">REPROGRAMMING COGNITIVE DECREE</span>
